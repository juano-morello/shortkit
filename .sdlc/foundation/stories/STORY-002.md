...
- [ ] AC-6: Given the API running from the production image built by the repository's `Dockerfile`, when `GET /health` is requested, then it returns 200 with a JSON body containing a `status` field equal to `"ok"` and a `commit` field matching the git SHA the image was built from.

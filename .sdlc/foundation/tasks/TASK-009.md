...
## ⚠ DEFERRED 2026-08-09 — your red step already exists, and it was reverted on purpose

This TASK left the surviving initiative when `launch-core` was re-scoped to EPIC-001
alone. EPIC-002 is now a named entry in `.sdlc/roadmap.md` with no ids and no design.
**Nothing here is cancelled.** The id is frozen, this card is intact, and every ⚠
section below still applies when EPIC-002 becomes its own initiative.

**The red step was written, committed, and then removed from the tree.**

- Red step landed: **`df45595`** — `test(auth): failing tests for AC-16 account half, AC-20, AC-21 [TASK-009]`
- Reverted: `apps/api/test/auth/credential-auth.int-spec.ts` (9 tests), deleted in the
  re-scope commit. Recover it verbatim with
  `git show df45595:apps/api/test/auth/credential-auth.int-spec.ts`.
- **Not reverted, and not yours:** `apps/api/test/support/api-server.ts` and
  `apps/api/test/support/auth-fixture.ts` are still on disk. `test/support/**` appears in
  no TASK's paths and belongs to `sdlc-test-architect` under routing rule 0 — the banner
  in `api-server.ts` says so. Both are also cited by surviving EPIC-001 files:
  `framework-400-request-body.spec.ts` reasons about `api-server.ts` to justify being a
  `.spec.ts` rather than an `.int-spec.ts`, and `rls-fixture.ts` cites `auth-fixture.ts`
  as precedent for the migration requirement. `auth-fixture.ts` has no importer until
  this TASK returns; that is deliberate, not dead code awaiting cleanup.

Why the spec was removed rather than skipped: a deferred TASK's failing tests left in the
tree mean the surviving increment can never report a green `pnpm test:integration`, and
every Ship-phase claim would inherit a caveat about failures outside its own scope.

The 10 findings filed against this TASK stay in `findings.yaml` — it is append-only — and
are parked with the deferral rather than closed.

...
| required in | `NODE_ENV === 'production'` only |
...
export const TRUSTED_CLIENT_IP_HEADER_UNSET_MESSAGE =
  'TRUSTED_CLIENT_IP_HEADER is not set. No client address can be trusted without it. See design/contracts/trusted-client-address.md.';
...
/**
 * Production-only. Asserts a trusted header name is DECLARED and well formed. Does not,
 * and cannot, assert that the named header is stripped by the hop in front.
 *
 * Throws Error with one of the three constants above. The message NEVER interpolates the
 * configured value: an environment read is not eligible for error text (ADR-0029).
 */
...
2. The value matches `TRUSTED_CLIENT_IP_HEADER_PATTERN` and is not in
   `FORBIDDEN_TRUSTED_HEADERS`. In production rule 2 cannot fail, because the boot assertion
   already refused. In development it can, and it returns `null` rather than throwing.
...
6. In production the process does not reach `listen()` unless a trusted header is declared and
   well formed.
...
- The integration suite declares `TRUSTED_CLIENT_IP_HEADER=x-test-client-ip` so the IP buckets
  are exercisable. Without it F-025's six-different-client-IPs test passes vacuously: it
  proves the email bucket fired without the IP bucket, and the IP bucket could not have fired
  under any input.

## Versioning

`TRUSTED_CLIENT_IP_HEADER` is configuration, changeable by redeploy. Changing it resets no
state: rate-limit keys expire within their window and `ip_hash` continuity is already tied to
`CLICK_IP_HASH_KEY`, not to the header.

 ...
 * ADR: adr-0012 (degradation posture), adr-0040 (a `null` principal), adr-0013
 * Produced by: TASK-1b-07 (wave 1 of item 1b) — the `@Public()` per-IP half.
 *              TASK-051 adds `checkTenant` and the Redis-backed implementation.
 *
 * ============================================================================
 * THE PORT `RateLimitGuard` CALLS. ONE METHOD TODAY, ON PURPOSE.
 * ============================================================================
 *
 * The contract names two buckets for the guard: the tenant-keyed write bucket (120 per 60 s,
 * `POST`/`PATCH`/`PUT`/`DELETE` on authenticated routes) and the IP-keyed bucket on `@Public()`
 * routes (30 per 60 s, every method). Item 1b ships the second because F-018 must be closed
 * before the first `@Public()` route that opens a tenant transaction exists (the invitation
 * lookup). The first is TASK-051's (D-08), and its `checkTenant` method is DELIBERATELY NOT
 * DECLARED HERE: a no-op method on the port would read as coverage that does not exist. The
 * constants it will use are declared below, quoted from the contract, so TASK-051 finds them
 * in the file the contract names as normative rather than restating them.
 *
...
/** The tenant-keyed write bucket's window (`rate-limit.md`, "Limits"). TASK-051's; declared, unused here. */
export const RATE_LIMIT_WINDOW_S = 60;

/** The tenant-keyed write bucket's limit per window (`rate-limit.md`, "Limits"). TASK-051's; declared, unused here. */

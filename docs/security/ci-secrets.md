...
The reason is what the value is used *for* on each side. On Vercel it is real: the API
performs a constant-time match on it before honouring a forwarded `x-shortkit-client-ip`,
so publishing it lets anyone forge that header and collapse every IP-keyed rate-limit
bucket into one shared by the whole product (ADR-0014,
`design/contracts/web-api-client.md`). In CI it is a **search needle** and nothing else:
`assert-no-inlined-secrets.mjs` needs some conforming value present in the environment so
it has something to look for in the built output. CI never talks to Fly. A random
base64url string does the job exactly as well.
